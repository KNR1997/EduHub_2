      ...
      <table className="w-full">
          ...
          {table.getHeaderGroups().map((headerGroup) => (
            <tr key={headerGroup.id}>
              {headerGroup.headers.map((header) => (
                <th
                  className="p-1 py-2  tracking-wide text-left"
                  key={header.id}
                >
                  {flexRender(
        ...
        <tbody>
          {table.getRowModel().rows.map((row) => (
            <tr
              className="odd:bg-activeNavLinkHover even:bg-activeNavLink "
              key={row.id}
            >
              {row.getVisibleCells().map((cell) => (
                <td
                  data-cell={`${cell.column.columnDef.header} : `}
                  className='p-1 py-2 tracking-wide text-left block lg:table-cell before:content-[attr(data-cell)] before:font-semibold lg:before:content-[""]'
                  key={cell.id}
                >
